...
(function() {
    ...
    function copySprintIssuesToClipboard($issues, sprintTitle, boardLink){
        ...
        $issues.find(".js-issue").not('.ghx-filtered').each(function(i){
            ...
            var spent = $($(this).find(".ghx-extra-field")[0]).text().replace("h", ""), // First look for the time spent
                spentText = "";

            if (spent.toLowerCase() !== "none")
            {
                spentText = " <span> - " + spent + " hs </span>";
            }
            ...
            content +=
                ...
                " <span>[<a href='https://azurecom.atlassian.net/browse/"+ $(this).data("issue-key") + "'>"+ $(this).data("issue-key") +"</a>]</span>" +
                spentText +
                "</p></li>";
        });
